...
def load_test_dataset():
    ...
    test_ds = test_ds.map(
        lambda image, label: (
            tf.keras.applications.mobilenet_v2.preprocess_input(tf.cast(image, tf.float32)),
            label
        )
    )
...
def save_confusion_matrix(y_true, y_pred, class_names):
    ...
    save_path = RESULT_DIR / "confusion_matrix" / "mobilenetv2_confusion_matrix.png"
    fig.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)


def main():
    model_path = MODEL_DIR / "mobilenetv2.keras"
    ...
    with open(
        RESULT_DIR / "reports" / "mobilenetv2_classification_report.txt",
        "w",
        encoding="utf-8"
    ) as f:
        f.write(report)
